...
To power desalination, the system needed to push water through a reverse osmosis membrane — and that meant generating high pressures, typically around 50–60 bar. We didn’t want to burn efficiency by converting mechanical wave energy into electricity and then back into pressure using pumps. Instead, we opted for a purely mechanical solution: the flap’s motion drove a piston pump directly.
...
Peak output pressures reached 8 bar — enough to validate the mechanical path from wave to pressure, even if not quite enough for real desalination. It was a solid first step — and one that gave us real data to build the next iteration on.
